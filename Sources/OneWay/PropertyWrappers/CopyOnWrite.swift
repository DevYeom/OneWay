...
/// A property wrapper that provides copy-on-write semantics to eliminate the cost of copying
/// large values.
///
/// When applied to a field, the value will always be allocated on the heap because this wrapper
/// uses a class, and classes in Swift are always heap-allocated. This is especially useful for
/// large value types, as it prevents stack overflow by managing the value on the heap instead of
/// the stack. Additionally, the copy-on-write behavior helps avoid unnecessary copying when the
/// value is not modified, thereby improving performance.
///
